...
fn test_assert_06b() {
    let ast = check_parse("function f() -> ():\n assert /* nothing */ true");
    assert_eq!(ast.get(1),&Node::from(BoolExpr(true)));
    assert_eq!(ast.get(2),&Node::from(BlockComment("/* nothing */".to_string())));
    assert_eq!(ast.get(3),&Node::from(AssertStmt(Expr(1))));
}
...
fn test_assert_06c() {
    let ast = check_parse("function f() -> ():\n assert // nothing\n    true");
    assert_eq!(ast.get(1),&Node::from(BoolExpr(true)));
    assert_eq!(ast.get(2),&Node::from(LineComment("// nothing".to_string())));
    assert_eq!(ast.get(3),&Node::from(AssertStmt(Expr(1))));
}
