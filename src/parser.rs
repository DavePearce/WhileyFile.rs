...
impl<'a> Parser<'a> {
    ...
    pub fn parse_decl_property(&mut self, _modifiers: Vec<decl::Modifier>) -> Result<Decl> {
        todo![];
    }
    ...
    fn finalise<T:Copy+Into<usize>>(&mut self, node:T, _span: Span<Token>) -> Result<T> {
